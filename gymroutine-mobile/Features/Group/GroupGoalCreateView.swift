...
struct GroupGoalCreateView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            analyticsService.logScreenView(screenName: "GroupGoalCreate")
    ...
    // MARK: - Goal Type Card
    private func goalTypeCard(for goalType: GroupGoalType) -> some View {
        ZStack {
            Button(action: {
    ...
    private var datePickers: some View {
        VStack(spacing: 12) {
            ...
            DatePicker("終了日", selection: $viewModel.endDate, in: viewModel.startDate..., displayedComponents: .date)
                .datePickerStyle(CompactDatePickerStyle())
                .disabled(viewModel.selectedRepeatOption != .none)
        }
    }
    ...
    // MARK: - Repeat Options Grid
    private var repeatOptionsGrid: some View {
        LazyVGrid(columns: [
            GridItem(.flexible()),
            GridItem(.flexible()),
            GridItem(.flexible())
            ...
            ForEach(RepeatOption.allCases, id: \.self) { option in
                repeatOptionCard(for: option)
            }
        }
    }
    
    // MARK: - Repeat Option Card
    private func repeatOptionCard(for option: RepeatOption) -> some View {
        Button(action: {
    ...
    // MARK: - Repeat Count Section
    private var repeatCountSection: some View {
        VStack(spacing: 12) {
            HStack {
