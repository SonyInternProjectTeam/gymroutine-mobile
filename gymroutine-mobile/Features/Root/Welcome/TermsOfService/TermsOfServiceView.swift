...
D.　第18条（利用制限および登録抹消）に定める措置を受けたことがある場合
...
• 本サービスにおけるユーザーの個人認証
• 本サービスにおけるユーザーへのサービスの提供（ワークアウトの記録・計画、運動結果の分析とパフォーマンス管理、カレンダー表示によるルーティン管理、他のユーザーの運動分析データの閲覧機能の提供を含みます）
• 当方または提携する広告配信事業者、フィットネス関連企業等の第三者が、ユーザーの属性情報、運動データ、利用傾向に基づき、最適化された商品広告を本サービス内または外部のウェブサイト・アプリに配信または表示するため
• 本サービスによるサービスの向上等を目的としたアンケート、キャンペーンの実施、またはユーザー満足度向上のための調査
• 本サービスからのメールによる各種情報の無料提供、お問い合わせへの返答
• 利用規約に違反する態様、スパム行為、不正アクセス等の不正利用の防止
• アクセス状況の把握、ニーズに適したサービスの提供ならびに改善（ユーザーの運動データや利用傾向の分析、本サービスの機能改善を含みます）
""")
                .font(.body)
...
• 本サービスのサービス向上の目的で個人情報を集計及び分析により得られたものを、個人を識別又は特定できない態様（例：特定の個人が識別できないよう統計処理された運動量や回数の平均値など）にて提携先等第三者に開示又は提供する場合、または他のユーザーが閲覧可能な形で共有される場合
...
• ユーザー自らが本サービス上の機能（他のユーザーへのデータ公開設定など）または別の手段を用いて、他のユーザーまたは第三者に個人情報を開示した場合
...
""")
            ...
            Text(
"""
本ポリシーが日本語と日本語以外の言語にて作成された場合、当該プライバシーポリシーの言語間での矛盾又は相違が生じた場合は、全ての点において日本語を優先するものとします。
""")
                .font(.body)
